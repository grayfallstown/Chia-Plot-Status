...
namespace ChiaPlotStatusLib.Logic.Parser
{
    class CPPlotLogFileParser
    {
        ...
        protected CPPlotLog CPPlotLog { get; } = new();
        ...
        public CPPlotLogFileParser(string path, bool closeOnEndOfFile)
        {
            ...
            CPPlotLog.StartDate = File.GetCreationTime(path);
            CPPlotLog.LogFile = this.LogFile;
            CPPlotLog.LogFolder = this.LogFolder;
            this.TailLineEmitter = new TailLineEmitter(path, closeOnEndOfFile, (line) =>
            {
                lineRead = true;
                CPPlotLog.LastLogLine = line.Trim();
                if (!firstRead)
                ...
                switch (line)
                {
                    case var _ when threadsRg.IsMatch(line):
                        ...
                        CPPlotLog.Threads = int.Parse(matches[0].Groups[1].Value);
                        CPPlotLog.EnterPhase(1);
                        break;
                    case var _ when bucketsRg.IsMatch(line):
                        matches = bucketsRg.Matches(line);
                        CPPlotLog.Buckets = int.Parse(matches[0].Groups[1].Value);
                        break;
                    case var _ when plotNameRg.IsMatch(line):
                        matches = plotNameRg.Matches(line);
                        CPPlotLog.PlotName = matches[0].Groups[1].Value;
                        break;
                    case var _ when tmp1Rg.IsMatch(line):
                        matches = tmp1Rg.Matches(line);
                        CPPlotLog.Tmp1Drive = matches[0].Groups[1].Value;
                        break;
                    case var _ when tmp2Rg.IsMatch(line):
                        matches = tmp2Rg.Matches(line);
                        CPPlotLog.Tmp2Drive = matches[0].Groups[1].Value;
                        break;
                    case var _ when lossRg.IsMatch(line):
                        matches = lossRg.Matches(line);
                        long lost = long.Parse(matches[0].Groups[1].Value);
                        switch (CPPlotLog.CurrentTable)
                        {
                            case 1:
                                CPPlotLog.P1Table1Lost = lost;
                                break;
                            case 2:
                                CPPlotLog.P1Table2Lost = lost;
                                break;
                            case 3:
                                CPPlotLog.P1Table3Lost = lost;
                                break;
                            case 4:
                                CPPlotLog.P1Table4Lost = lost;
                                break;
                            case 5:
                                CPPlotLog.P1Table5Lost = lost;
                                break;
                            case 6:
                                CPPlotLog.P1Table6Lost = lost;
                                break;
                            case 7:
                                CPPlotLog.P1Table7Lost = lost;
                                break;
                        }
                        if (CPPlotLog.P1Table1Lost == 0)
                            CPPlotLog.P1Table1Lost = lost;
                        else if (CPPlotLog.P1Table2Lost == 0)
                            CPPlotLog.P1Table2Lost = lost;
                        else if (CPPlotLog.P1Table3Lost == 0)
                            CPPlotLog.P1Table3Lost = lost;
                        else if (CPPlotLog.P1Table4Lost == 0)
                            CPPlotLog.P1Table4Lost = lost;
                        else if (CPPlotLog.P1Table5Lost == 0)
                            CPPlotLog.P1Table5Lost = lost;
                        else if (CPPlotLog.P1Table6Lost == 0)
                            CPPlotLog.P1Table6Lost = lost;
                        else if (CPPlotLog.P1Table7Lost == 0)
                            CPPlotLog.P1Table7Lost = lost;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p1table1Rg.IsMatch(line):
                        matches = p1table1Rg.Matches(line);
                        CPPlotLog.P1Table1 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.CurrentTable = 2;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p1table2Rg.IsMatch(line):
                        matches = p1table2Rg.Matches(line);
                        CPPlotLog.P1Table2 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P1Table2Found = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 3;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p1table3Rg.IsMatch(line):
                        matches = p1table3Rg.Matches(line);
                        CPPlotLog.P1Table3 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P1Table3Found = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 4;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p1table4Rg.IsMatch(line):
                        matches = p1table4Rg.Matches(line);
                        CPPlotLog.P1Table4 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P1Table4Found = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 5;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p1table5Rg.IsMatch(line):
                        matches = p1table5Rg.Matches(line);
                        CPPlotLog.P1Table5 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P1Table5Found = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 6;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p1table6Rg.IsMatch(line):
                        matches = p1table6Rg.Matches(line);
                        CPPlotLog.P1Table6 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P1Table6Found = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 7;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p1table7Rg.IsMatch(line):
                        matches = p1table7Rg.Matches(line);
                        CPPlotLog.P1Table7 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P1Table7Found = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p1timeRg.IsMatch(line):
                        matches = p1timeRg.Matches(line);
                        CPPlotLog.P1 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.EnterPhase(2);
                        break;
                    case var _ when p2timeRg.IsMatch(line):
                        matches = p2timeRg.Matches(line);
                        CPPlotLog.P2 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.EnterPhase(3);
                        break;
                    case var _ when p3timeRg.IsMatch(line):
                        matches = p3timeRg.Matches(line);
                        CPPlotLog.P3 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P3Entries = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.EnterPhase(4);
                        break;
                    case var _ when p4timeRg.IsMatch(line):
                        matches = p4timeRg.Matches(line);
                        CPPlotLog.P4 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.FinalFileSize = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.EnterPhase(5);
                        break;
                    case var _ when totaltimeRg.IsMatch(line):
                        matches = totaltimeRg.Matches(line);
                        CPPlotLog.Total = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.EnterPhase(6);
                        this.Close();
                        break;
                    ...
                    case var _ when p2MaxTableSize.IsMatch(line):
                        ...
                        CPPlotLog.P2MaxTableSize = long.Parse(matches[0].Groups[1].Value);
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table1scanRg.IsMatch(line):
                        matches = p2table1scanRg.Matches(line);
                        CPPlotLog.P2Table1Scan = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.CurrentTable = 1;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table1rewriteRg.IsMatch(line):
                        matches = p2table1rewriteRg.Matches(line);
                        CPPlotLog.P2Table1Rewrite = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P2Table2Lost = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 2;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table2scanRg.IsMatch(line):
                        matches = p2table2scanRg.Matches(line);
                        CPPlotLog.P2Table2Scan = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.CurrentTable = 2;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table2rewriteRg.IsMatch(line):
                        matches = p2table2rewriteRg.Matches(line);
                        CPPlotLog.P2Table2Rewrite = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P2Table2Lost = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 3;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table3scanRg.IsMatch(line):
                        ...
                        CPPlotLog.P2Table3Scan = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.CurrentTable = 3;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table3rewriteRg.IsMatch(line):
                        matches = p2table3rewriteRg.Matches(line);
                        CPPlotLog.P2Table3Rewrite = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P2Table3Lost = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 4;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table4scanRg.IsMatch(line):
                        matches = p2table4scanRg.Matches(line);
                        CPPlotLog.P2Table4Scan = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.CurrentTable = 4;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table4rewriteRg.IsMatch(line):
                        matches = p2table4rewriteRg.Matches(line);
                        CPPlotLog.P2Table4Rewrite = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P2Table4Lost = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 5;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table5scanRg.IsMatch(line):
                        matches = p2table5scanRg.Matches(line);
                        CPPlotLog.P2Table5Scan = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.CurrentTable = 5;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table5rewriteRg.IsMatch(line):
                        matches = p2table5rewriteRg.Matches(line);
                        CPPlotLog.P2Table5Rewrite = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P2Table5Lost = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 6;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table6scanRg.IsMatch(line):
                        matches = p2table6scanRg.Matches(line);
                        CPPlotLog.P2Table6Scan = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.CurrentTable = 6;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table6rewriteRg.IsMatch(line):
                        matches = p2table6rewriteRg.Matches(line);
                        CPPlotLog.P2Table6Rewrite = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P2Table6Lost = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.CurrentTable = 7;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table7scanRg.IsMatch(line):
                        matches = p2table7scanRg.Matches(line);
                        CPPlotLog.P2Table7Scan = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.CurrentTable = 7;
                        CPPlotLog.NextPhasePart();
                        break;
                    case var _ when p2table7rewriteRg.IsMatch(line):
                        matches = p2table7rewriteRg.Matches(line);
                        CPPlotLog.P2Table7Rewrite = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P2Table7Lost = long.Parse(matches[0].Groups[2].Value);
                    ...
                    case var _ when p31table3Rg.IsMatch(line):
                        ...
                        CPPlotLog.P31Table3 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P31Table3Entries = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.NextPhasePart();
                        CPPlotLog.CurrentTable = 4;
                        break;
                    case var _ when p31table4Rg.IsMatch(line):
                        matches = p31table4Rg.Matches(line);
                        CPPlotLog.P31Table4 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P31Table4Entries = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.NextPhasePart();
                        CPPlotLog.CurrentTable = 5;
                        break;
                    case var _ when p31table5Rg.IsMatch(line):
                        matches = p31table5Rg.Matches(line);
                        CPPlotLog.P31Table5 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P31Table5Entries = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.NextPhasePart();
                        CPPlotLog.CurrentTable = 6;
                        break;
                    case var _ when p31table6Rg.IsMatch(line):
                        matches = p31table6Rg.Matches(line);
                        CPPlotLog.P31Table6 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P31Table6Entries = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.NextPhasePart();
                        CPPlotLog.CurrentTable = 7;
                        break;
                    case var _ when p31table7Rg.IsMatch(line):
                        matches = p31table7Rg.Matches(line);
                        CPPlotLog.P31Table7 = float.Parse(matches[0].Groups[1].Value, CultureInfo.InvariantCulture);
                        CPPlotLog.P31Table7Entries = long.Parse(matches[0].Groups[2].Value);
                        CPPlotLog.NextPhasePart();
                        CPPlotLog.CurrentTable = 0;
                        break;
                    case var _ when p31table2Rg.IsMatch(line):
         ...
         */
        public CPPlotLog ParseCPPlotLog()
        {
            if (closed)
                return CPPlotLog;
            lineRead = false;
            this.TailLineEmitter.ReadMore();
            CPPlotLog.FileLastWritten = File.GetLastWriteTime(this.LogFile);
            if (CPPlotLog.FileLastWritten != null && ((DateTime.Now - (DateTime)CPPlotLog.FileLastWritten).TotalDays > 1d))
                Close();
            if (lineRead && !firstRead)
                lastGrown = DateTime.Now;
            if (lastGrown != null)
                CPPlotLog.FileLastWritten = lastGrown;
            firstRead = false;
